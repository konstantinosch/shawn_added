...
typedef wiselib::WiselibMessage<node_id_t, block_data_t, long> WiselibMessage_c;
...
namespace shawn
{
        ...
        throw()
    {
             ...
             endit = world_w().end_adjacent_nodes_w( *mi.src_ ); it != endit; ++it )
        {
           ...
           {
        	   const WiselibMessage_c* message = dynamic_cast<const WiselibMessage_c*> ( mi.msg_.get() );
        	   message->payload_size();
        	   message->max_len();
        	   //if ( message->payload_size() > message->max_len() )
        	   //{
        	   //printf( "%ld\t%ld\n", message->payload_size(), message->max_len() );
        	   //}
