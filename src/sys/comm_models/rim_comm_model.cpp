...
/*		for (int j = 0; j < sequence_count_; j++ ) 
		...
		const throw()
	{
		...
		//if ( distance < range_ * min_range_ * vsp_ * u.transmission_range()) return true;
		//if ( distance > range_ * max_range_ * vsp_ * u.transmission_range()) return false;
		// Calculate transmission range
		double a = angle( u.real_position(), v.real_position() );
		
		double i = interpolate_Ki(a, u.id());
		double PX = shadow_log_fade(distance, u.transmission_range()*range_ );
		//cout << "sender="<< u.id() << " receiver=" << v.id() <<" d=" << distance <<  " interpol="<<i<<" vsp:" << vsp_<< " a=" << a << " source_transm_range= " << u.transmission_range() <<  " product=" << range_ * interpolate_Ki(a, u.id()) * vsp_ * u.transmission_range() << endl;
		//return  distance <= range_ * interpolate_Ki(a, u.id());
		return ( rand() % 100 <= PX * 100 * interpolate_Ki(a, u.id()) );
	}
...
double RimCommModel::
shadow_log_fade( double x, double Range )
const throw()
{
